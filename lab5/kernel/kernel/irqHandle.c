...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		if(Flags>0x2&&(Flags>>2)!=destInode.type)
		{
				putString("the file type not match\n");
				sf->eax=-1;
				return;
		}
		...
		for(i=0;i<MAX_FILE_NUM;i++){
			...
			{
				putString("the file had been openned\n");
				sf->eax=-1;
				return;
			}
		}
		for(i=0;i<MAX_DEV_NUM;i++){
			...
			{
				putString("the dev file had been openned\n");
				sf->eax=-1;
				return;
			}
		}
		...
		for(i=0;i<MAX_FILE_NUM;i++){
			if(file[i].state==0){
				...
				putString("Successfully open the file!\n");
				return;
			}
		}
		...
		if(!(Flags & O_CREATE))
		{
			putString("file not exist ,and the O_CREATE not be set\n");
			sf->eax = -1;
			return;
		}
		...
		if ((sf->edx >> 3) % 2 == 0) { 
			...
			{
				putString("file not exist ,and the father path is not exist too\n");
				sf->eax = -1;
				return;
			}
			...
			if (ret == -1)
			{
				putString("不存在这个文件,并且需要创建的文件的路径不存在");
				sf->eax = -1;
				return;
			}
...
void syscallWrite(struct StackFrame *sf) {
	...
	if(sf->ecx!=STD_OUT)
	{
		if (sf->ecx < MAX_DEV_NUM || sf->ecx >  MAX_DEV_NUM + MAX_FILE_NUM||file[sf->ecx - MAX_DEV_NUM].state == 0) 
		{
			...
			putString("syscallWrite: the file not open or exceed the limit\n");
			sf->eax=-1;
			return;
		}
	}
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	MemCpy(str,&buffer[0]+remainder,sz);
...
void syscallRead(struct StackFrame *sf) {
	...
	if(sf->ecx!=STD_OUT)
	{
		if (sf->ecx < MAX_DEV_NUM || sf->ecx >  MAX_DEV_NUM + MAX_FILE_NUM||file[sf->ecx - MAX_DEV_NUM].state == 0) 
		{
			putString("syscallRead: the file not open or exceed the limit\n");
			sf->eax=-1;
			return;
		}
	}
...
void syscallReadFile(struct StackFrame *sf) {
	...
	while(stroff<sz)
	{
		...
		if(j==0)
		{
			if(sBlock.blockSize-remainder>=sz)
				{
					MemCpy(&buffer[0]+remainder,str,sz);
					stroff=sz;
				}
			else
			{
				MemCpy(&buffer[0]+remainder,str,sBlock.blockSize-remainder);
				stroff+=(sBlock.blockSize-remainder);
			}
		}
		else
		{
				if(sBlock.blockSize+stroff>=sz)
				{
					MemCpy(&buffer[0],(str)+stroff,sz-stroff);
					stroff=sz;
				}
				else
				{
					MemCpy(&buffer[0],(str)+stroff,sBlock.blockSize);
	...
	putString((char*)str);
	putChar('\n');
	putString("Successfully read the file!\n");
	pcb[current].regs.eax = sz;
	file[sf->ecx - MAX_DEV_NUM].offset += sz;
	return;
}
...
void syscallClose(struct StackFrame *sf) {
	...
	putString("Successfully close the file!\n");
...
void syscallRemove(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		{
			putString("the file is being using\n");
			pcb[current].regs.eax = -1;
			return;
		}
